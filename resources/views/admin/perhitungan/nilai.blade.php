    ...
    <div class="col-md-12">
        <div class="box box-success">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-clipboard"></i> Nilai </h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            
            <form class="form-horizontal" action="/superadmin/perhitungan/subkriteria/{{$data->id}}" method="post">
                @csrf
                <div class="box-body">
                    ...
                    <div class="col-sm-10">
                      <select class="form-control" name="subkriteria_id">
                        @foreach ($subkriteria as $item)
                            <option value="{{$item->id}}" {{$data->subkriteria_id == $item->id ? 'selected':''}}>{{$item->nama}} - nilai profil : {{$item->nilai}}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
